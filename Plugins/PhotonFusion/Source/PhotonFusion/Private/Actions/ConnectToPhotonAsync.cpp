...
UConnectToPhotonAsync* UConnectToPhotonAsync::ConnectToPhoton(const FFusionConnectOptions ConnectOptions, UObject* WorldContextObject)
{
	UConnectToPhotonAsync* action = NewObject<UConnectToPhotonAsync>();
	...
	if (const FWorldContext* WorldContext = GEngine->GetWorldContextFromWorld(WorldContextObject->GetWorld()))
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("Try connecting from instance type: %s"), *UEnum::GetValueAsString(WorldContextType))
	}
	
	return action;
}

void UConnectToPhotonAsync::Activate()
{
	if (bFailInstantly)
	{
		...
		if (OnFailure.IsBound()) {
			OnFailure.Broadcast(EActionFailureCodes::InvalidRegion);
		}
...
void UConnectToPhotonAsync::TryConnect()
{
	...
	if (!Settings->UseLocalServer && ConnectOptions.RegionSelectionMode == EFusionRegionSelectionMode::Select)
	{
	...
	TimerManager.SetTimer(TimerState.Handle, this, &UConnectToPhotonAsync::WaitConnect, CheckInterval, true);
}

void UConnectToPhotonAsync::WaitConnect()
{
	FTimerManager& TimerManager = WorldContextObjectBase->GetWorld()->GetTimerManager();
	const UGameInstance* GameInstance = UGameplayStatics::GetGameInstance(WorldContextObjectBase);
	if (!GameInstance)
	...
	if (ConnectPhase == EConnectPhase::WaitingForRegions && PendingRegionsTask.has_value())
	{
		if (PendingRegionsTask->IsReady())
		{
			...
			if (!RegionsResult.IsOk())
			{
				...
				TimerManager.ClearTimer(TimerState.Handle);
				if (OnFailure.IsBound())
					OnFailure.Broadcast(EActionFailureCodes::Error);
				DestroyAction();
				return;
			}
