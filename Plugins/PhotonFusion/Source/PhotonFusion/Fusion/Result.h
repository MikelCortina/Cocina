...
namespace PhotonMatchmaking {
    ...
    public:
        ...
        ErrorCode GetErrorCode() const {
            return IsErr() ? std::get<1>(data).code : ErrorCode::Ok;
        }

        template<typename F>
        auto Transform(F&& f) const& -> Result<std::invoke_result_t<F, const T&>> {
            using U = std::invoke_result_t<F, const T&>;
            if (IsErr()) return Result<U>::Err(GetError());
            if constexpr (std::is_void_v<U>) {
                std::invoke(std::forward<F>(f), GetValue());
                return Result<U>::Ok();
            } else {
                return Result<U>::Ok(std::invoke(std::forward<F>(f), GetValue()));
            }
        }
        ...
        auto Transform(F&& f) && -> Result<std::invoke_result_t<F, T&&>> {
            using U = std::invoke_result_t<F, T&&>;
            if (IsErr()) return Result<U>::Err(std::move(*this).GetError());
            if constexpr (std::is_void_v<U>) {
                std::invoke(std::forward<F>(f), std::move(*this).GetValue());
                return Result<U>::Ok();
            } else {
                return Result<U>::Ok(std::invoke(std::forward<F>(f), std::move(*this).GetValue()));
            }
        }
        ...
        template<typename F>
        auto OrElse(F&& f) const& -> std::invoke_result_t<F, const Error&> {
            using RetType = std::invoke_result_t<F, const Error&>;
        ...
        template<typename F>
        auto OrElse(F&& f) & -> std::invoke_result_t<F, const Error&> {
            using RetType = std::invoke_result_t<F, const Error&>;
            static_assert(std::is_same_v<RetType, Result>,
                "OrElse: callable must return Result<T> (same T)");
            if (IsOk()) return Result::Ok(GetValue());
        ...
        template<typename F>
        auto OrElse(F&& f) && -> std::invoke_result_t<F, Error&&> {
            using RetType = std::invoke_result_t<F, Error&&>;
            static_assert(std::is_same_v<RetType, Result>,
                "OrElse: callable must return Result<T> (same T)");
        ...
        auto TransformError(F&& f) const& -> Result {
            static_assert(std::is_same_v<std::invoke_result_t<F, const Error&>, Error>,
        ...
        template<typename F>
        auto TransformError(F&& f) && -> Result {
            static_assert(std::is_same_v<std::invoke_result_t<F, Error&&>, Error>,
                "TransformError: callable must return Error");
            if (IsOk()) return Result::Ok(std::move(*this).GetValue());
    ...
    private:
        ...
        explicit Result(Error error) : data(std::in_place_index<1>, std::move(error)) {}
    ...
    public:
        ...
        ErrorCode GetErrorCode() const {
            return IsErr() ? errorOpt->code : ErrorCode::Ok;
        }

        template<typename F>
        auto Transform(F&& f) const& -> Result<std::invoke_result_t<F>> {
        ...
        template<typename F>
        auto Transform(F&& f) && -> Result<std::invoke_result_t<F>> {
            using U = std::invoke_result_t<F>;
            if (IsErr()) return Result<U>::Err(std::move(*this).GetError());
            if constexpr (std::is_void_v<U>) {
                std::invoke(std::forward<F>(f));
    ...
    private:
        Result() = default;
        explicit Result(Error error) : errorOpt(std::move(error)) {}

        std::optional<Error> errorOpt;
    };
} // namespace PhotonMatchmaking
